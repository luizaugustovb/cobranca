        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="lg:col-span-2 space-y-8">
                    ...
                    <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-xl border border-gray-100 dark:border-gray-700 overflow-x-auto">
                        <div class="px-8 py-5 border-b border-gray-100 dark:border-gray-700 flex items-center gap-2">
                            ...
                            <h3 class="font-black text-slate-800 dark:text-white uppercase tracking-tighter text-lg">Titulos Negociados</h3>
                            <span class="ml-auto text-xs font-black text-gray-400 uppercase tracking-widest bg-blue-50 dark:bg-blue-900/20 text-blue-500 px-3 py-1 rounded-full">
                                {{ $acordo->titulos->count() }} titulo(s)
                            </span>
                        </div>
                        <table class="min-w-full divide-y divide-gray-100 dark:divide-gray-700">
                            <thead class="bg-gray-50 dark:bg-gray-700/30">
                                <tr>
                                    <th class="px-6 py-3 text-left text-[10px] font-black uppercase tracking-widest text-gray-400">No Titulo</th>
                                    ...
                                    <th class="px-6 py-3 text-right text-[10px] font-black uppercase tracking-widest text-gray-400">Honorarios</th>
                            ...
                            <tbody class="divide-y divide-gray-50 dark:divide-gray-700/50">
                                ...
                                <tr class="bg-blue-100/50 dark:bg-blue-900/20 font-black">
                                    <td colspan="5" class="px-6 py-3 text-right text-xs font-black uppercase tracking-widest text-blue-600">Total dos titulos</td>
                        ...
                        <table class="min-w-full divide-y divide-gray-100 dark:divide-gray-700">
                            ...
                            <tbody class="divide-y divide-gray-50 dark:divide-gray-700">
                                ...
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                                    ...
                                    <td class="px-6 py-4 text-sm text-gray-500 uppercase">{{ $pgto->forma_pagamento ?? '&mdash;' }}</td>
                                    <td class="px-6 py-4 text-xs text-gray-400 font-mono">{{ $pgto->gateway_id ?? '&mdash;' }}</td>
                                </tr>
                                @endforeach
                            </tbody>
                        </table>
                ...
                <div class="space-y-6">
                    ...
                    'BOLETO' => ['icon' => 'boleto', 'label' => 'Boleto Bancario', 'color' => 'bg-blue-50 dark:bg-blue-900/20 border-blue-200'],
                    'PIX' => ['icon' => 'pix', 'label' => 'Pix', 'color' => 'bg-green-50 dark:bg-green-900/20 border-green-200'],
                    'CREDIT_CARD' => ['icon' => 'credit_card', 'label' => 'Cartao de Credito', 'color' => 'bg-blue-50 dark:bg-blue-900/20 border-blue-200'],
                    default => ['icon' => 'livre', 'label' => 'Livre (devedor escolhe)', 'color' => 'bg-amber-50 dark:bg-amber-900/20 border-amber-200'],
                    };
                    @endphp
                    ...
                    {{-- Card: Link de Pagamento / Chave PIX --}}
                    @if($acordo->asaas_link)
                    @if($acordo->forma_pagamento === 'PIX')
                    <div class="bg-emerald-600 rounded-2xl p-6 shadow-xl shadow-emerald-500/30">
                        <p class="text-xs font-black text-emerald-200 uppercase tracking-widest mb-3">Chave PIX</p>
                        <p class="text-sm text-white font-black break-all mb-1">{{ $acordo->asaas_link }}</p>
                        <p class="text-xs text-emerald-200 mb-4">Pagamento à vista — sem cobrança no Asaas.</p>
                        <button onclick="navigator.clipboard.writeText('{{ $acordo->asaas_link }}')"
                    ...
                    @if($acordo->devedor->telefone)
                    <form action="{{ route('tenant.acordos.whatsapp', $acordo) }}" method="POST">
                        @csrf
                        <button type="submit"
                            class="w-full flex items-center justify-center gap-2 px-4 py-3 bg-green-500 hover:bg-green-600 text-white text-xs font-black uppercase tracking-widest rounded-xl transition shadow-lg shadow-green-500/20">
                            <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                                <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z" />
                                <path d="M12 0C5.373 0 0 5.373 0 12c0 2.127.558 4.126 1.533 5.858L.054 23.214a.75.75 0 00.928.928l5.356-1.479A11.945 11.945 0 0012 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 22c-1.93 0-3.734-.526-5.276-1.442l-.378-.224-3.928 1.083 1.083-3.928-.224-.378A9.96 9.96 0 012 12C2 6.477 6.477 2 12 2s10 4.477 10 10-4.477 10-10 10z" />
                            </svg>
                    ...
                    <a href="{{ route('tenant.devedores.show', $acordo->devedor) }}"
                        ...
                        <svg class="w-4 h-4 text-gray-300 group-hover:text-blue-400 ml-auto transition" fill="none" viewBox="0 0 24 24" stroke="currentColor">
