<x-app-layout>
    <x-slot name="header">
        <h2 class="font-black text-3xl text-slate-800 dark:text-white flex items-center tracking-tighter uppercase leading-none">
            Nova Importação de Cobranças
        </h2>
        <p class="text-xs text-gray-400 font-bold tracking-widest mt-1">Envie o arquivo com responsáveis, alunos e títulos em um único lote.</p>
    </x-slot>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-2xl p-10 border border-gray-100 dark:border-gray-700">
                <form action="{{ route('tenant.importacoes.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="space-y-8">
                        ...
                        <div class="space-y-4">
                            ...
                            <label class="flex flex-col items-center justify-center w-full h-56 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-3xl cursor-pointer bg-gray-50 dark:bg-gray-700 hover:bg-indigo-50 dark:hover:bg-gray-600 transition-all duration-200">
                                <div class="flex flex-col items-center justify-center text-center px-4">
                                    <svg class="w-12 h-12 mb-3 text-indigo-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 20 16">
                                        <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 13h3a3 3 0 0 0 0-6h-.025A5.56 5.56 0 0 0 16 6.5 5.5 5.5 0 0 0 5.207 5.021C5.137 5.017 5.071 5 5 5a4 4 0 0 0 0 8h2.167M10 15V6m0 0L8 8m2-2 2 2"/>
                                    </svg>
                                    <p class="text-sm text-gray-500 dark:text-gray-400 font-semibold">
                                        <span class="text-indigo-600 font-black">Clique para selecionar</span> ou arraste o arquivo
                                    </p>
                                    <p class="text-xs text-gray-400 mt-1">XLSX, XLS ou CSV &mdash; máx. 10 MB</p>
                                    <p id="file-name" class="text-xs text-indigo-600 font-bold mt-2 hidden"></p>
                                </div>
                                <input id="arquivo" name="arquivo" type="file" accept=".xlsx,.xls,.csv" class="hidden" required
                        ...
                        {{-- Tem PDF? Botão para abrir modal --}}
                        <div class="flex items-center gap-3 p-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700 rounded-2xl">
                            ...
                            <p class="text-xs text-amber-700 dark:text-amber-400 font-semibold flex-1">
                                Seu arquivo está em PDF?
                                <span class="text-gray-400 font-normal">Converta para XLSX primeiro e depois importe aqui.</span>
                            </p>
                            <button type="button" onclick="document.getElementById('modal-pdf').classList.remove('hidden')"
                                class="inline-flex items-center gap-2 px-4 py-2 bg-amber-500 hover:bg-amber-600 text-white text-xs font-black uppercase tracking-widest rounded-xl shadow transition flex-shrink-0">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-3xl shadow p-8 border border-gray-100 dark:border-gray-700">
                <div class="flex items-center justify-between mb-6">
                    <div>
                        ...
                        <p class="text-xs text-gray-400 mt-1">Cada linha representa um título. Informe o cabeçalho exatamente conforme abaixo.</p>
                    </div>
                    <a href="{{ route('tenant.importacoes.template') }}"
                        class="inline-flex items-center gap-2 px-5 py-3 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-black uppercase tracking-widest rounded-2xl shadow transition">
                    ...
                    <table class="w-full text-xs text-left">
                        <thead>
                            <tr class="bg-gray-50 dark:bg-gray-700">
                                ...
                                <th class="px-4 py-2 font-black uppercase tracking-widest text-gray-600 dark:text-gray-300">Obrigatório</th>
                                <th class="px-4 py-2 font-black uppercase tracking-widest text-gray-600 dark:text-gray-300 rounded-r-xl">Descrição / Exemplo</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                            @php
                            $colunas = [
                                ['responsavel',   true,  'Nome completo do responsável financeiro', 'João da Silva'],
                                ['cpf',           true,  'CPF do responsável (com ou sem pontuação)', '123.456.789-09'],
                                ['contato',       false, 'Telefone/WhatsApp (apenas números ou formatado)', '(11) 91234-5678'],
                                ['email',         false, 'E-mail do responsável', 'joao@escola.com'],
                                ['rua',           false, 'Logradouro do endereço', 'Rua das Flores'],
                                ['numero_end',    false, 'Número do endereço', '123'],
                                ['cep',           false, 'CEP (apenas números)', '01310100'],
                                ['aluno',         false, 'Nome do aluno/beneficiário', 'Maria da Silva'],
                                ['matricula',     false, 'Matrícula do aluno', 'MAT001'],
                                ['servico',       false, 'Serviço prestado / descrição do título', 'Mensalidade Escolar'],
                                ['numero_titulo', true,  'Número único do título', 'TIT-2024-001'],
                                ['vencimento',    true,  'Data de vencimento (dd/mm/aaaa)', '31/12/2024'],
                                ['valor',         true,  'Valor principal do título', '1500,00'],
                                ['multa',         false, 'Percentual ou valor de multa', '2,00'],
                                ['juros',         false, 'Percentual ou valor de juros', '1,00'],
                                ['honorarios',    false, 'Honorários advocatícios (valor fixo)', '10,00'],
                            ];
                            @endphp
                            @foreach ($colunas as [$col, $req, $desc, $ex])
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-750">
                                ...
                                <td class="px-4 py-2">
                                    ...
                                    @else
                                        <span class="bg-gray-100 text-gray-500 dark:bg-gray-700 dark:text-gray-400 px-2 py-0.5 rounded-full font-black text-[10px] uppercase">Não</span>
                                    @endif
                                </td>
                ...
                <p class="mt-4 text-xs text-gray-400">
                    <strong>Dica:</strong> Cada linha = um título. Se um responsável tiver múltiplos títulos, repita os dados do responsável em cada linha.
                    O sistema agrupa automaticamente pelo CPF, evitando duplicatas.
                </p>
            </div>
                ...
                <form id="form-pdf" action="{{ route('tenant.pdf-conversao.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="space-y-5">
                        <label class="flex flex-col items-center justify-center w-full h-40 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-2xl cursor-pointer bg-gray-50 dark:bg-gray-700 hover:bg-red-50 dark:hover:bg-gray-600 transition-all">
                            <div class="flex flex-col items-center text-center px-4">
                                ...
                                <p class="text-sm text-gray-500 dark:text-gray-400">
                                    <span class="text-red-500 font-black">Clique para selecionar</span> o PDF
                                </p>
                                <p class="text-xs text-gray-400 mt-1">Apenas PDF &mdash; máx. 20 MB</p>
                                <p id="pdf-name" class="text-xs text-red-600 font-bold mt-1 hidden"></p>
                            </div>
                            <input id="arquivo-pdf" name="arquivo" type="file" accept=".pdf" class="hidden" required
                                onchange="document.getElementById('pdf-name').textContent = this.files[0]?.name; document.getElementById('pdf-name').classList.remove('hidden');" />
                        </label>
                        ...
                        <p class="text-xs text-gray-400 text-center">
                            O download do XLSX iniciará automaticamente após a conversão.
                        </p>

                        <div class="flex gap-3">
                            <button type="button" onclick="fecharModal()"
                                class="flex-1 px-4 py-3 bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-gray-200 dark:hover:bg-gray-600 transition">
                                Cancelar
                            </button>
                            <button type="submit" id="btn-pdf"
                                class="flex-1 inline-flex items-center justify-center gap-2 px-4 py-3 bg-amber-500 hover:bg-amber-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow transition">
                                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/></svg>
                                Converter e Baixar
                            </button>
                        </div>
                        ...
                        <div class="space-y-4">
                            ...
                            <label class="flex flex-col items-center justify-center w-full h-56 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-3xl cursor-pointer bg-gray-50 dark:bg-gray-700 hover:bg-indigo-50 dark:hover:bg-gray-600 transition-all duration-200">
                                <div class="flex flex-col items-center justify-center text-center px-4">
                                    ...
                                    <p class="text-xs text-gray-400 mt-1">XLSX, XLS ou CSV — máx. 10 MB</p>
                                    <p id="file-name" class="text-xs text-indigo-600 font-bold mt-2 hidden"></p>
                                </div>
                                <input id="arquivo" name="arquivo" type="file" accept=".xlsx,.xls,.csv" class="hidden" required
                                    onchange="document.getElementById('file-name').textContent = this.files[0]?.name; document.getElementById('file-name').classList.remove('hidden');" />
                    ...
                    <table class="w-full text-xs text-left">
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-750">
                                ...
                                <td class="px-4 py-2 text-gray-500 dark:text-gray-400">{{ $desc }} <span class="italic text-gray-400">— ex: {{ $ex }}</span></td>
                            </tr>
                            @endforeach
                        </tbody>
                    </table>
