<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center">
            ...
            <h2 class="font-black text-lg sm:text-2xl text-slate-800 uppercase tracking-tighter">Editar UsuÃ¡rio</h2>
        </div>
    </x-slot>
    ...
    <div class="py-6 sm:py-12">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white rounded-3xl shadow-xl border border-slate-100 p-8">
                ...
                <form action="{{ route('tenant.usuarios.update', $user) }}" method="POST" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-2">
                            ...
                            <x-text-input id="name" name="name" type="text" class="w-full bg-slate-50 border-slate-100 rounded-2xl py-4" :value="old('name', $user->name)" required />
                        </div>
                        <div class="space-y-2">
                            <x-input-label for="email" value="E-mail" class="text-[10px] font-black uppercase tracking-widest text-slate-400" />
                            <x-text-input id="email" name="email" type="email" class="w-full bg-slate-50 border-slate-100 rounded-2xl py-4" :value="old('email', $user->email)" required />
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-2">
                            ...
                            <x-text-input id="phone" name="phone" type="text" class="w-full bg-slate-50 border-slate-100 rounded-2xl py-4" :value="old('phone', $user->phone)" placeholder="(84) 99999-9999" />
                        </div>
                        <div class="space-y-2">
                    ...
                    <div>
                        <p class="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-4">Nova Senha (deixe em branco para manter a atual)</p>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="space-y-2">
                                <x-input-label for="password" value="Nova Senha" class="text-[10px] font-black uppercase tracking-widest text-slate-400" />
                                <x-text-input id="password" name="password" type="password" class="w-full bg-slate-50 border-slate-100 rounded-2xl py-4" placeholder="MÃ­nimo 8 caracteres" />
                            </div>
                            <div class="space-y-2">
                                ...
                                <x-text-input id="password_confirmation" name="password_confirmation" type="password" class="w-full bg-slate-50 border-slate-100 rounded-2xl py-4" placeholder="Repita a nova senha" />
                            </div>
                        </div>
                    </div>
